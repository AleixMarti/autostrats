...
namespace NinjaTrader.Strategy
{
    ...
    /// </summary>
    [Description("")]
    public class TFL3BRATM : Strategy
    {
        #region Variables
        ...
        protected override void OnBarUpdate()
        {
			...
			
			{
				...
				AtmStrategyCreate(Cbi.OrderAction.Buy, OrderType.Stop, 0, High[1] + PrevBarsPlusTicks * TickSize, TimeInForce.Day, orderId, "TF_long_3W_3BR", atmStrategyId);	
			...
			if (orderId.Length > 0)
			{
				...
				//{
					if
					(status[2] == "Accepted" || status[2] == "Working" || status[2] == "Pending" && Close [0] - 5 * TickSize > High[1])
                					
				    AtmStrategyCancelEntryOrder(orderId);
					Print("98 TFL3BRATM Cancelled Order less than 5 ticks");
			...
			if (atmStrategyId.Length > 0 && GetAtmStrategyMarketPosition(atmStrategyId) == MarketPosition.Flat)
				{
				Print("125 reset the strategy id");
				atmStrategyId = string.Empty;
				orderId = string.Empty;
				}
